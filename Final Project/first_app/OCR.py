...
def pdf_ocr(path, langs):
    ...
    image_counter = 1
    for page in pages:
        filename = "media/page" + str(image_counter) + ".jpg"
    ...
    for i in range(1, image_counter):
        filename = "media/page" + str(i) + ".jpg"
        img = Image_filter(filename)
        text = str(pytesseract.image_to_string(img, config=langs))
        f_text += text
    return f_text, image_counter-1



def img_ocr(file, langs):
    img = Image_filter(file)
    filename = "media/page" + '1' + ".jpg"
    plt.imsave(filename, img,cmap='gray')

    out = pytesseract.image_to_string(img, config=langs)
    return out, 1
